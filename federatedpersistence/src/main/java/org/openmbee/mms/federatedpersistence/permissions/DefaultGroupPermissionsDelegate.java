...
public class DefaultGroupPermissionsDelegate extends AbstractDefaultPermissionsDelegate {
    ...
    public boolean hasPermission(String user, Set<String> groupPerms, String privilege) {
        ...
        if (!group.getType().equals("local") && privilege.equalsIgnoreCase("GROUP_EDIT")) {
        ...
        if (!groupPerms.isEmpty() && groupGroupPermRepo.existsByGroupAndGroupPerm_NameInAndRoleIn(group, groupPerms, roles)) {
            return true;
        }
        return false;
    }
    ...
    @Override
    public boolean hasGroupPermissions(String groupPerm, String privilege) {
        for (GroupGroupPerm perm: groupGroupPermRepo.findAllByGroup_GroupName(group.getName())) {
            if (perm.getGroup().getName().equals(groupPerm) && perm.getRole().getPrivileges().stream()
