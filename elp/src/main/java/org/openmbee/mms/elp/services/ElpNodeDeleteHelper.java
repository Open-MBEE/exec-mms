...
@Service("elpNodeDeleteHelper")
@Scope(
    value = ConfigurableBeanFactory.SCOPE_PROTOTYPE,
    proxyMode = ScopedProxyMode.TARGET_CLASS)
public class ElpNodeDeleteHelper extends NodeDeleteHelper {
    private static final Logger logger = LoggerFactory.getLogger(ElpNodeDeleteHelper.class);
    ...
    protected boolean validateOperation(Operation operation, Map<String, Object> newElement, Map<String, Object> existingElement, boolean overwrite) {
        ...
        if(operation == Operation.REMOVE) {
            //Check for write access in element
            if(Collections.disjoint(userGroups, existingElementWriteGroups)) {
                return false;
            }
            return true;
        }
