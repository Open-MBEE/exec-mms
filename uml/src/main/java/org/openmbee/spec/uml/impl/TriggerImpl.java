...
@Entity
@SecondaryTable(name = "Trigger")
@Table(appliesTo = "Trigger", fetch = FetchMode.SELECT, optional = false)
@DiscriminatorValue(value = "Trigger")
@JsonTypeName(value = "Trigger")
public class TriggerImpl extends MofObjectImpl implements Trigger {

    private Element owner;
    private String name;
    private VisibilityKind visibility;
    private Collection<Port> port;
    ...
    @Column(name = "name", table = "Trigger")
    ...
    @ManyToAny(metaDef = "PortMetaDef", metaColumn = @Column(name = "portType"), fetch = FetchType.LAZY)
    @JoinTable(name = "Trigger_port",
        joinColumns = @JoinColumn(name = "TriggerId"),
        inverseJoinColumns = @JoinColumn(name = "portId"))
    public Collection<Port> getPort() {
        if (port == null) {
            port = new ArrayList<>();
        }
        return port;
    ...
    public void setPort(Collection<Port> port) {
        this.port = port;
    }
    ...
    @JsonGetter
    @JsonSerialize(using = MofObjectSerializer.class)
    @Any(metaDef = "EventMetaDef", metaColumn = @Column(name = "eventType"), fetch = FetchType.LAZY)
    @JoinColumn(name = "eventId", table = "Trigger")
    ...
    @JsonProperty(required = true)
    @JsonSetter
    @JsonDeserialize(using = MofObjectDeserializer.class, as = EventImpl.class)
    ...
    @JoinColumn(name = "nameExpressionId", table = "Trigger")
    ...
    @JoinTable(name = "Trigger_ownedComment",
        joinColumns = @JoinColumn(name = "TriggerId"),
